	...
	for _, r := range registry {
		...
		case ciTag != "":
			if len(tags) == 4 {
				...
				login(docker, r)
				deploy(docker, tags[1]+"-"+arch, r)

				if !ignoredSuffixes.MatchString(ciTag) {
					deploy(docker, tags[2]+"-"+arch, r)
					deploy(docker, tags[3]+"-"+arch, r)
					deploy(docker, "latest-"+arch, r)
				}
			} else {
		...
		case ciBranch != masterTag && !publicRepo.MatchString(ciBranch):
			login(docker, r)
			deploy(docker, ciBranch+"-"+arch, r)
		case ciBranch != masterTag && publicRepo.MatchString(ciBranch):
			login(docker, r)
			deploy(docker, "PR"+ciPullRequest+"-"+arch, r)
		case ciBranch == masterTag && ciPullRequest == stringFalse:
			login(docker, r)
			deploy(docker, "master-"+arch, r)
		default:
	...
	for _, r := range registry {
		...
		case ciTag != "":
			if len(tags) == 4 {
				log.Infof("Detected tags: '%s' | '%s' | '%s'", tags[1], tags[2], tags[3])
				login(docker, r)
				deployManifest(docker, tags[1], tags[1]+"-amd64", tags[1]+"-arm32v7", tags[1]+"-arm64v8", r)
				publishDockerReadme(docker)

				if !ignoredSuffixes.MatchString(ciTag) {
					deployManifest(docker, tags[2], tags[2]+"-amd64", tags[2]+"-arm32v7", tags[2]+"-arm64v8", r)
					deployManifest(docker, tags[3], tags[3]+"-amd64", tags[3]+"-arm32v7", tags[3]+"-arm64v8", r)
					deployManifest(docker, "latest", "latest-amd64", "latest-arm32v7", "latest-arm64v8", r)
					publishDockerReadme(docker)
		...
		case ciBranch != masterTag && !publicRepo.MatchString(ciBranch):
			login(docker, r)
			deployManifest(docker, ciBranch, ciBranch+"-amd64", ciBranch+"-arm32v7", ciBranch+"-arm64v8", r)
		case ciBranch != masterTag && publicRepo.MatchString(ciBranch):
			login(docker, r)
			deployManifest(docker, "PR"+ciPullRequest, "PR"+ciPullRequest+"-amd64", "PR"+ciPullRequest+"-arm32v7", "PR"+ciPullRequest+"-arm64v8", r)
		case ciBranch == masterTag && ciPullRequest == stringFalse:
			login(docker, r)
			deployManifest(docker, "master", "master-amd64", "master-arm32v7", "master-arm64v8", r)
			publishDockerReadme(docker)
		default:
